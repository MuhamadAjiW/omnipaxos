...
use omnipaxos::{
    ...
    ClusterConfigEC, OmniPaxosEC, OmniPaxosECConfig,
};
...
type MemoryStore = Arc<Mutex<MemoryStorage<TestECEntry, ClusterConfigEC>>>;
...
fn _setup_leader() -> (
    ...
    let n_old = n;
    let setup_msg = Message::<TestECEntry, ClusterConfigEC>::BLE(BLEMessage {
        from: 2,
        to: 1,
    ...
    let setup_msg = Message::<TestECEntry, ClusterConfigEC>::SequencePaxos(PaxosMessage {
        from: 2,
        to: 1,
...
fn ec_atomic_storage_acceptsync_test() {
    fn run_single_test(fail_after_n_ops: usize) {
        ...
        let msg = Message::<TestECEntry, ClusterConfigEC>::SequencePaxos(PaxosMessage {
            from: 2,
            to: 1,
            msg: PaxosMsg::AcceptSync(AcceptSync {
                n,
