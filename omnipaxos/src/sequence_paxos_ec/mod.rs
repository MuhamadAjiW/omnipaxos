...
{
    pub(crate) internal_storage: InternalStorage<B, T, ClusterConfigEC>,
    pid: NodeId,
    peers: Vec<NodeId>, // excluding self pid
    state: (RoleEC, PhaseEC),
    buffered_proposals: Vec<T>,
    buffered_stopsign: Option<StopSign<ClusterConfigEC>>,
    outgoing: Vec<Message<T, ClusterConfigEC>>,
    leader_state: LeaderState<T, ClusterConfigEC>,
    latest_accepted_meta: Option<(Ballot, usize)>,
    // Keeps track of sequence of accepts from leader where AcceptSync = 1
    current_seq_num: SequenceNumber,
    cached_promise_message: Option<Promise<T, ClusterConfigEC>>,
    #[cfg(feature = "logging")]
    logger: Logger,
    ...
    B: Storage<T, ClusterConfigEC>,
{
    ...
    pub(crate) fn with(config: SequencePaxosConfigEC, storage: B) -> Self {
        ...
        let mut paxos = SequencePaxosEC {
            ...
            outgoing,
            leader_state: LeaderState::<T, ClusterConfigEC>::with(leader, max_pid, quorum),
    ...
    /// the buffer.
    pub(crate) fn take_outgoing_msgs(&mut self, buffer: &mut Vec<Message<T, ClusterConfigEC>>) {
        if buffer.is_empty() {
    ...
    pub(crate) fn handle(&mut self, m: PaxosMessage<T, ClusterConfigEC>) {
        match m.msg {
    ...
    pub(crate) fn is_reconfigured(&self) -> Option<StopSign<ClusterConfigEC>> {
    ...
    pub(crate) fn get_leader_state(&self) -> &LeaderState<T, ClusterConfigEC> {
        &self.leader_state
    }
    ...
    pub(crate) fn forward_stopsign(&mut self, ss: StopSign<ClusterConfigEC>) {
        let leader = self.get_current_leader();
        if leader > 0 && self.pid != leader {
            #[cfg(feature = "logging")]
